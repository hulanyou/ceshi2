...
function mOck(thisObj, v) {
	...
	if (onoff == '0') {
		...
		if ($(thisObj).hasClass("unover") == false) {
			$(thisObj).addClass("selday").siblings().removeClass("selday").parent(".add_date").siblings().children("td").removeClass("selday");
...
//周历-日期
	var now = new Date();
	//日期-周历-日数
	var week_date = now.getDate(); //获取当前日
	var n_y = now.getFullYear(); //当前年
	var n_m = now.getMonth() + 1; //获取当前月份 
	var n_days = DayNumOfMonth(n_y, n_m); //获取当前月份天数
	var zYue = $('#yue').html() //日历月
	var zNian = $('#nian').html() //日历年
$(function() {
	var week_date = now.getDate(); //获取当前日
	var n_y = now.getFullYear(); //当前年
	var n_m = now.getMonth() + 1; //获取当前月份 
	...
	var zYue = $('#yue').html() //日历月
	var zNian = $('#nian').html() //日历年
	$("#week_nian span").html(now.getFullYear());
	$("#week_yue span").html(now.getMonth() + 1);
	$("#week_zhou span").html(theWeek());
	if (zYue == n_m && zNian == n_y) {
		 //获取当月1号星期几 
		now.setDate(1);
		var zWeek = now.getDay() - 1;
		for (i = 0; i < (week_date - 1); i++) {
			var rr = $("#SD" + i).html();
			$(".add_date td").eq(i + zWeek+1).addClass("no_choice");
			$(".add_date td").eq(zWeek - i).removeClass("no_choice");
		}
		for (var j = 0; j < n_days; j++) {
			$(".add_date td").eq(week_date + zWeek + j).removeClass("no_choice");
		}
	}
	$(".add_date td").eq(week_date+zWeek).addClass("selday");
	$(".ymNaviBtn").click(function() {
		var zYue = $('#yue').html() //日历月
		var zNian = $('#nian').html() //日历年
		 //获取当月1号星期几 
		now.setDate(1);
		...
		if (zNian < n_y) {
			$(".add_date td").addClass("no_choice");
		} else if (zNian > n_y) {
			$(".add_date td").removeClass("no_choice");
		} else if (zYue > n_m && zNian == n_y) {
			$(".add_date td").removeClass("no_choice");
		} else if (zYue < n_m && zNian == n_y) {
			$(".add_date td").addClass("no_choice");
		} else if (zYue == n_m && zNian == n_y) {
		...
		if ($(".add_date td").hasClass("unover")) {
			$(this).removeClass("no_choice");
		}
	});

});

function DayNumOfMonth(Year, Month) { //返回当月天数
...
//上一周
var now1 = new Date();
var sTime = now1.getTime();

function PreWeek(dat) {
